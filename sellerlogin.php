...
<?php
        if (isset($_POST['login']))
        {
            ...
            $password_store = $_POST['password_store'];
            $select = mysqli_query($con,"Select * FROM store WHERE store_id='$store_id' AND password_store='$password_store'");
            ...
            if(is_array($row))
            {
              ...
              error_reporting(E_ALL);
                ...
                $_SESSION["password_store"] = $row ['password_store'];
            ...
            {
                echo '<script type="text/javascript">
                location.replace("http://localhost/Project/sellerspage.php"); </script>';
            }
        }
?>
<!doctype html>
  ...
  <body>
    ...
    <div class="row pt-3 pb-5 justify-content-evenly" style="background-color:#bed8ec">
        <div class="col-md-5 mt-5">
          <h4 class="display-6 pb-3 text-center"><b>Log in As A Seller.</b></h4>
          <form method="post">
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Id</label>
              <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="store_id">
              <div id="emailHelp" class="form-text">Provide your Id.</div>
            </div>
    
            <div class="mb-3">
              <label for="inputmessage" class="form-label">Password</label>
              <input type="password" class="form-control" id="exampleInputmessgage" name="password_store">
            </div>
            <button name="login"value="login"type="submit" class="btn btn-primary">Submit</button>
            <h3 class="display-8 mt-5"><a href="login.html">Login as a buyer.</a></h3>
            <h3 class="display-8 mt-5"><a href="Admin.html">Admin Login.</a></h3>
          </form>
        </div>
        </div>